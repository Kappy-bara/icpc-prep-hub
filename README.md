...
10,000**, and **Average user** (rating bands, sampled). That data is **not**
fetched live per visitor, and it is **not** derived from the problem set —
an earlier version of this tried bucketing *problems* by rating as a proxy
for "what a typical player at this tier solves," which produced numbers
that got misread as real solve counts (e.g. a "baseline pool" of 1,662
problems is not "the average user solves 1,662 problems"). It's fixed now:
every tier's numbers come from real players. For Top 500 / Top 10,000 /
Average user, a small sample (10 by default) of real rated users whose
*current rating* falls in that band gets its solve history fetched and
averaged — both the overall solved count and the tag mix. Tourist is the
same technique with a sample size of exactly one, by name.

[`js/cf-baseline-data.js`](js/cf-baseline-data.js) is the committed static
snapshot, generated by
[`scripts/generate-cf-baseline.js`](scripts/generate-cf-baseline.js):

```bash
node scripts/generate-cf-baseline.js
```
...
(for real, current rank-500/rank-10,000/median rating cutoffs, a
percentile-lookup table used by the rating-trajectory percentile, and the
sampling pools for each tier), the contest list (for the last-year window),
then fetches each sampled player's submission history in turn (~30 requests
total at the default sample size, ~2s apart per Codeforces' rate-limit
courtesy) and overwrites `js/cf-baseline-data.js` with the averaged results.
Review the console output (sample sizes, average solved counts, top tags
per tier) before committing — the average-solved-count numbers should
increase monotonically from Average user → Top 10,000 → Top 500 → Tourist;
...
manual step by design (see [`TODO.md`](TODO.md) for the idea of automating
it via a scheduled Supabase Edge Function instead).
