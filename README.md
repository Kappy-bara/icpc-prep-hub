...
The Codeforces Analysis page's "your tag mix vs. the baseline" comparison
needs to know what tag ratios are naturally common at different skill tiers
on Codeforces. That data is **not** fetched live per visitor — it's far too
heavy (every rated user, every contest, every problem) to do on every page
load. Instead, [`js/cf-baseline-data.js`](js/cf-baseline-data.js) is a
committed static snapshot, generated by
